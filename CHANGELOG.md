...
- Park the parent turn after a successful public async launch on compatible Pi hosts, preserving the active work checkpoint until native completion or real input resumes it instead of starting text-only continuation and status polling.

- Treat a successfully completed Pi `finish_work` summary as the child's final output, so summary-only work completion is persisted and returned instead of being reported as an empty `toolUse` response.
