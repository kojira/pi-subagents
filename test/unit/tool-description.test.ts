	...
	it("keeps execution, authority, evidence and recovery contracts in every built-in mode", () => {
		for (const description of [DEFAULT_SUBAGENT_TOOL_DESCRIPTION, FULL_SUBAGENT_TOOL_DESCRIPTION, COMPACT_SUBAGENT_TOOL_DESCRIPTION]) {
			for (const contract of [
				...
				/exactly one top-level asynchronous subagent workflow call/,
				...
				/Native completion wakes this parent.*return control.*bg_wait only registers an immediate wake subscription/,
				/no async argument or synchronous execution option/,
