...
• Omit action for execution. For an authorized delegated multi-step/parallel workflow: exactly one top-level asynchronous subagent workflow call; children launch only inside it.
• All public subagent launches are asynchronous; there is no async argument or synchronous execution option. Keep the returned run identity. Consume results at dependency barriers. Native completion wakes this parent: return control rather than sleeping or polling. bg_wait only registers an immediate wake subscription for work without native notifications.
